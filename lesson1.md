...
Есть операционные системы,в которых успешное завершение программы - это не всегда 0, а завершение с ошибкой - не всегда 1. То есть успех и ошибка могут обозначаться разными числами. Такие значения обычно называют `implementation defined`, то есть, зависящими от реализации.Поэтому, чтобы ваша программа работала правильно на любой операционной система, можно использовать специальные слова: EXIT_SUCCESS и EXIT_FAILURE. Пока можно считать, что это переменные, в которых хранятся конкретные значения кодов возврата. EXIT_SUCCESS и EXIT_FAILUR записаны в специальном файле `<cstdlib>`, который нужно подключить с помощью **директивы** `#include`. Директивы - это инструкции препроцессору, все директивы начинаются с `#`. По внешнему эффекту действие `#include` похоже на `import` в питоне, но в реальности все сложнее, об этом поговорим позже.
...
Перед нами выражение, состоящее из 4х сущностей (`std::cout`, `<<` текста `std::endl`) и точки с запятой.

Выражение состоит из операций и операндов. Например `3 + 4` - это выражение с операцией `+` и операндами `3 и 4`. 
Обратите внимание, что наше выражение оканчивается `;`, зачем это нужно - на следующих уроках.
